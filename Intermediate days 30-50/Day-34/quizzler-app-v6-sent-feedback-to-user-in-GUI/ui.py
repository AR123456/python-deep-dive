...
# call next question from quiz_brain
    def get_next_question(self):
        # change bg color to or back to white
    ...
    def true_pressed(self):
        ...
        # can write on one line this way
        self.give_feedback( self.quiz.check_answer("True"))

    def false_pressed(self):
        #pass over False when false press is called
        # or can wright on 2 lines
        is_right = self.quiz.check_answer("False")
        self.give_feedback(is_right)
    def give_feedback(self, is_right):
        #change background to red or green , delay 1 sec and change to white, go to next question
        # cannot use time becuase of mainloop() so use the tkinter after function
        if is_right:
